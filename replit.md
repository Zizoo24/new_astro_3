...
- **CSS Architecture:** Uses CSS custom properties and `clamp()` for responsive design. Key files include `base-architecture.css` (design system tokens, responsive variables), `porto-desktop.css`, `navigation-glassmorphism.css`, `sticky-mobile.css`, `visibility-fixes.css`, and `dark-mode.css`.
