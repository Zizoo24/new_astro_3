...
- **Hero Section:** Desktop features three overlapping coral cards ("Trust Stack") with unified layout (icon circle + title + description): 1) DUL License (shield icon, "Official Dubai business registry – click to verify", "We are officially licensed" badge), 2) Government Accepted (building icon, logos on white pills for contrast), 3) 60-Minute Turnaround (bolt icon). Mobile displays cards sequentially at 92% opacity. Logos remain vibrant (no grayscale).
...
- **CSS Architecture:** Uses CSS custom properties and `clamp()` for responsive design. All CSS consolidated into single `public/styles/app.css` file (335KB, ~15,700 lines) for improved maintainability and reduced HTTP requests.
- **CSS Consolidation (Dec 2025):** All 13 individual CSS files merged into unified `app.css` with section headers preserving original load order: base-architecture → porto-desktop → services-enhanced → document-pages → visibility-fixes → navigation-glassmorphism → subsection-menu → sticky-mobile → mobile-ios → mobile-android → desktop-macos → dark-mode → megamenu. Additional files: `print.css` (print styles), `mobile-action-bar.css` (optional). Reduced !important declarations from 361 to 320. Navigation layer cleaned (42→1 !important). Homepage-specific header overrides maintain cascade priority. OS-agnostic base variables (--os-font, --os-radius, --os-nav-height, --os-elevation-*) inherited by platform-specific sections. Dark-mode.css analysis: 255 !important declarations required for light→dark cascade override; full refactor deferred as high-risk.
- **Dark Mode Contrast Fixes (Dec 2025):** Comprehensive dark mode overrides added to dark-mode.css for homepage sections: Exodus process steps (with proper coral gradient step-index), Why cards, Testimonials carousel (visible text on glass cards), section labels/titles, metrics strip, and specialists section. Mobile step-index visibility fixed in sticky-mobile.css (previously coral text on coral background, now white text on coral gradient). All class selectors verified to match actual component class names.
- **Visual Relief & Readability (Dec 2025):** Added dedicated CSS section (~270 lines) addressing content density issues: body line-height increased to 1.7 (with 1.2 overrides for compact UI elements), 75ch max-width for text blocks, section padding clamp(60px, 8vw, 100px), improved card gaps, FAQ answers reformatted with bold keywords and bullet points, specialist tags with border-top separator.
...
- **Mobile Sidebar:** Uses a navy glass background with `backdrop-filter: blur()`, ensuring WCAG AA contrast for text and icons using `var(--text-on-dark)` tokens. Refactored to true ARIA-compliant accordion pattern with semantic `<button>` elements for headings, `aria-expanded`/`aria-controls` attributes, and "View all" links inside each panel. Services accordion expanded by default. Lighter styling with reduced padding (12px) and smaller fonts (0.85rem).
- **Navigation Structure:** Desktop header includes About link between Resources and Contact. Mobile sidebar includes About & Licensing and Contact as single links at bottom. Footer quick links include About & Licensing.
