...
- **Global Scale:** `html { font-size: 80% }` applies a 20% reduction to all rem-based elements for a more compact desktop fit. Header uses flexbox layout with flexible nav width (`flex: 1 1 auto`) and fixed contact button (`flex-shrink: 0`). Media query at 1400px further reduces spacing for narrower screens.
- **Dark Mode:** Toggle sets both `body.theme-dark` class and `html[data-theme="dark"]` attribute. Color variables auto-switch in dark mode. Moon/sun icon toggle in header and mobile.
